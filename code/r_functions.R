  ...
  dat, outcome, factors, model = NULL, stats = NULL)
{
  ...
  # Varainces
    ...
    for(i in 1:length(factors)){
      ...
      print(car::leveneTest(as.formula(paste(outcome, '~',factors[i])),
                       data = dat,
                       center="mean"))
    }
...
r2 <- function(ss_num,ss_dem)
{
  (ss_num)/(ss_dem)
}
# compute R^2
simple_r2 <- function(fit.sum){
  k <- length(fit.sum[[1]]$`Mean Sq`)
  simpleR <- matrix(NA,ncol=1,nrow=(k-1))
  ss <- fit.sum[[1]]$`Sum Sq`
  sst <- sum(fit.sum[[1]]$`Sum Sq`)
  for(i in 1:(k-1))
  {
    simpleR[i,] <- round(r2(ss[i],sst),4)
  }
  rownames(simpleR) <- rownames(fit.sum[[1]])[1:(k-1)]
  colnames(simpleR) <- "r^2"
  return(simpleR)
}
