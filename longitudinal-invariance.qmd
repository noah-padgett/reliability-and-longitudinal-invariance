...
ss <- psych::describeBy(
  analysis.dat[,ITEMS],
  group=analysis.dat$wave,
  mat=T
) 
ss %>%
  mutate(
    item=substr(rownames(ss),1,3),
    Wave = group1
  )%>%
...
print(xtable(cbind(ss[,c("group1", "mean", "sd")], prop)))
```
...
p
ggsave("permutations.pdf", p, width=7.5, height=2.5, units = "in")
...
Backwards compared to thresholds, a lower mean on the latent response would indicate a lower response on average to the item. Meaning, items with a lower latent response variable average will have a lower average on the observed items, on average. The major gain we get from moving from the observed categorical indicators to the latent response variables is a easier comparison on a continuous scale as opposed to the observed categorical scale.
